...
def process(data_file, out_file, movie_set):
    ...
    with open(data_file, 'r', encoding='utf-8') as fin, open(out_file, 'w', encoding='utf-8') as fout:
        for line in tqdm(fin):
            ...
            userid_set.add(user_id + 1)
            userid_set.add(resp_id + 1)
            context, resp = [], ''
            entity_list = []
...
if __name__ == '__main__':
    ...
    valid_userid_set = process('valid_data_dbpedia.jsonl', 'valid_data_processed.jsonl', movie_set)
    test_userid_set = process('test_data_dbpedia.jsonl', 'test_data_processed.jsonl', movie_set)
    train_userid_set = process('train_data_dbpedia.jsonl', 'train_data_processed.jsonl', movie_set)

    userid_set.update(train_userid_set)
    userid_set.update(valid_userid_set)
    userid_set.update(test_userid_set)

    with open('item_ids.json', 'w', encoding='utf-8') as f:
        json.dump(list(movie_set), f, ensure_ascii=False)
    print(f'#movie: {len(movie_set)}')  # movie: 5552

    with open('user_ids.json', 'w', encoding='utf-8') as f:  # 从1开始
        # train+valid+test出现的所有人
        json.dump(list(userid_set), f, ensure_ascii=False)
    print(f'#user: {len(userid_set)}')  # user:1112
